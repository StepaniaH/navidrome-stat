...
- 仪表盘筛选条件与年度回顾范围保存在 URL 中，刷新不丢失、链接可分享。
...
| `STATS_QUERY_BUDGET_MS` | `250` | `/metrics` 使用的每个仪表盘子查询预算，限制在 10–60000 毫秒；用于观测回归，不会自动启用汇总表。 |
...
“最近播放”中的信息按钮会显示这类短会话占已观测播放尝试的比例。安装前回填和原生历史导入不属于应用实时观测到的会话，因此不进入该比例；从 Navidrome Stat 隐私归档恢复的记录则保留原有的计数属性。
...
发布容器会关闭请求访问日志，避免将应用 URL 中的仪表盘筛选条件、用户名和数据源标识写入容器日志；应用日志也不会输出播放元数据和上游请求 URL。自定义应用服务器、反向代理与 Navidrome 可能仍有各自的访问日志，分享日志前请检查相关配置。
