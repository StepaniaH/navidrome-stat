...
- 外观支持跟随系统、深色和浅色三种模式，以及 9 个配色家族、13 个具体变体。内置、Gruvbox、Catppuccin、Solarized 均有深浅变体；Nord、Dracula、Tokyo Night、Macchiato、Mocha 仅支持深色。选择只保存在当前浏览器，并同步应用于统计页、年度回顾、设置和 API 文档；另有七种界面语言及 Apple 风格语言选择器。
...
- 仪表盘支持按用户筛选；年度回顾图表可在播放次数与收听时长之间切换。
...
如需汇总多个服务器，请在启动后通过“设置 > 连接”逐个添加；保存的凭据会以 AES-256-GCM 静态加密，密钥存放在数据库旁的 `secret.key`（随安装生成）。请把该文件与数据库一并备份，否则仅恢复数据库副本后需要重新输入密码；该加密保护的是数据库文件与备份被 casual 检视的场景，不能防御主机完全失控。如果不能接受这种存储方式，请只使用环境变量配置的单一连接，不要通过设置页保存连接。
