...
本文件保存 `docs/tasks.md` 中状态为「已完成」的任务全文，供追溯验收记录。ID 永不复用。后续推进只在 [`tasks.md`](tasks.md) 进行；不要在本文件新增待办。

阅读顺序：先看 [`tasks.md`](tasks.md) 总览，再按 ID 回到此处查阅完成记录。完成记录里的「遗留」是当时快照；若已被后续任务消化，以 `tasks.md` 的「后续核验」和 [`current-state.md`](current-state.md) 为准。
...
- **后续核验（2026-08-21）**：采集器热更新已由 NDS-REL-002 实现。明文 GUI 密码仍是已记录的自托管权衡，见 [`privacy.md`](privacy.md)。
