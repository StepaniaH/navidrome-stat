...
Privacy exports use format v4. Every history row and short-play attempt carries a stable record ID plus a canonical SHA-256 fingerprint, so repeated merge imports report inserted, skipped, and conflicting records instead of duplicating them. Format v4 also freezes the duration-quality claim used by detail views, so restoring an archive cannot turn an estimate into a reported value or discard a lower-bound marker. Import formats v1 through v3 remain readable; v3 keeps its original signed fields and fingerprint rules but cannot reconstruct session/finalization evidence that was absent from the archive, while v1 and v2 derive deterministic identities during import.
