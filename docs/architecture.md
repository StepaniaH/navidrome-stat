...
| `js/dashboard/entity-detail.js` | URL-addressable artist/album dialog, scoped request lifecycle, trend chart, and safe detail rendering |

Dashboard filters, artist and album identities, and year-in-review scope use shareable URL parameters. Opening a detail view resolves the browser timezone token to an exact IANA name, preserves date, metric, server, and user scope, and restores through browser history. The dashboard carries the selected server, user, and resolved timezone into its review link; the review page restores those values together with the selected year, sends the same scope to the statistics API, and keeps the year, user, server, and timezone visible. Review requests expose explicit loading, empty, error, and retry states. Changing years aborts the previous request, and a generation check prevents an older response from replacing the current selection.
