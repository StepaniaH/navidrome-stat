...
3. Each collector calls the Subsonic `getNowPlaying` endpoint on a fixed interval. It also checks `getOpenSubsonicExtensions` for the `playbackReport` capability. An independently authenticated ListenBrainz-compatible receiver can accept opt-in push events from Navidrome.
...
| ListenBrainz payload validation and push receiver | `src/listenbrainz_ingest.py`, `src/routes/listenbrainz.py` |
...
| Statistics read queries by overview, time bucket, ranking, and history | `src/stats_query_*.py`; `src/stats_queries.py` compatibility module |
| Listening Review aggregation | `src/review_queries.py` |
...
Two optional importers fill the gap before live polling started. Both emit the same normalized listen event (provenance columns, unknown duration and transcoding left NULL, `duration_confidence` set to `unknown`) and write through a single idempotent path: `StatsService.record_imported_events` inserts with an `external_event_key`, whose partial unique index makes re-runs no-ops. Import rows use `source = 'backfill'` or `'song_history'`, and add plays without inflating listening minutes.

- **Backfill bridge**: each enabled server may point at a Navidrome smart playlist (`.nsp`). A watch task reads it through public `getPlaylist` on `BACKFILL_INTERVAL_SEC` and converts one timestamped event per track while leaving listened duration and transcoding unknown; only the newest play per track is exact, so older plays under `playCount` are never invented. A manual **sync** endpoint runs once immediately.
- **`getSongHistory` importer**: the adapter paginates the proposed OpenSubsonic endpoint ([upstream Navidrome PR #5650](https://github.com/navidrome/navidrome/pull/5650), not merged). It follows the proposal's current `count`/`offset` request and `songHistory.song` response contract, including Unix-second `playedAt` values and the endpoint's 500-row maximum. A missing or unknown non-empty response shape is an import failure rather than a successful empty page. The polling loop already probes for it; when a server advertises it, the initial import commits one page at a time, persists its per-source/user offset in SQLite, resumes limited batches after restarts, and backs off after failures.
...
## ListenBrainz-compatible receiver

When `LISTENBRAINZ_INGEST_TOKEN` and `LISTENBRAINZ_INGEST_USERNAME` are set, `/1/validate-token` and `/1/submit-listens` accept scrobbles from Navidrome. These routes use a separate token in the `Authorization` header and check it before reading a submission body. `playing_now` is validated but not stored. `single` and `import` events use the existing import write path.

The retry key includes the receiver source, configured username, timestamp, recording identity, and release identity. Recording MBIDs or MSIDs are used when present; otherwise a hash of the artist and title is used. Received records leave listening duration and transcoding status unset. Polling and import channels keep their own records, so an operator should configure one live collection method per user unless separate records are wanted.
...
Privacy exports use format v5. Artist credits are preserved alongside each recording and included in its fingerprint when available. Every history row and short-play attempt carries a stable record ID plus a canonical SHA-256 fingerprint, so repeated merge imports report inserted, skipped, and conflicting records instead of duplicating them. Format v4 also preserves the duration-quality metadata returned by the statistics API. Import formats v1 through v4 remain readable; v3 keeps its original signed fields and fingerprint rules but cannot reconstruct session or finalization data that was absent from the archive, while v1 and v2 derive deterministic identities during import.
...
Dashboard history is served by aggregate queries. A validated, immutable `StatsScope` is shared by cache keys and repository query conditions so date, timezone, metric, server, and user filters stay consistent. Summary and Listening Review responses include duration coverage, duration quality, and collection-source counts for API consumers. Dashboard and Listening Review totals display the stored listening-time value without estimate or minimum prefixes and use an em dash when no duration was recorded.

Artist, album, and client detail queries reuse the same scope for totals, unique-track and per-play averages, local-date trends, top tracks, recent plays, and equal-length prior-period ranks. Album identities include their source so same-named records cannot merge across servers. Duration fields retain a quality value in the API: `reported`, `estimated`, `lower_bound`, or `unknown`. Poller rows without a session ID predate durable checkpoints, and unfinished durable rows contain only their latest checkpoint. Startup recovery records those interrupted checkpoints as `lower_bound`, and privacy export/import preserves that value.

The relationship query returns ranked time series, four local-time dayparts, and current/previous-period values for artists, albums, or clients. Its chart contract is documented in [Dashboard relationships and drill-downs](data-relations.md). `StatsReadRepository` runs detail and relationship reads inside one SQLite read transaction, while the dashboard snapshot is assembled from one database snapshot. Rows that need IANA timezone and daylight-saving calculations are streamed through Python. A short-lived in-process cache covers identical filters and relationship dimensions, and successful writes request cache invalidation.

Below-threshold playback-attempt totals are loaded only when the Recent Plays information panel is opened. The query uses the dashboard's date, server, user, and timezone filters. Its rate compares these attempts with live-poller sessions and privacy-archive restores; backfill and native-history imports are excluded because they were not observed as live playback attempts.
...
| `themes.css` | Shared semantic theme tokens for the dashboard, Listening Review, settings, and API reference |
...
Dashboard filters, the selected relationship dimension, artist and album identities, and Listening Review scope use shareable URL parameters. Opening an artist or album detail resolves the browser timezone token to an exact IANA name, preserves date, metric, server, and user scope, and restores through browser history. Client details preserve the same scope but remain page-local and use a POST body so the client name is not stored in the URL or query-string access logs. The dashboard carries the selected server, user, and resolved timezone into its review link; the review page restores those values together with the selected calendar period, sends the same scope to the statistics API, and keeps the period, user, server, and timezone visible. Year mode renders monthly buckets; month mode renders daily buckets and both expose previous-period play comparison. Review requests expose explicit loading, empty, error, and retry states. Changing a period aborts the previous request, and a generation check prevents an older response from replacing the current selection.
...
- Dashboard authentication is optional. `STATS_API_TOKEN` grants administrator access; `STATS_READ_ONLY_TOKEN` grants statistics and Listening Review access and can be bound to one source and/or username. The backend applies the fixed scope to queries, server options, and cover-art source checks. Protected metrics require administrator access. The ListenBrainz ingestion token is separate and does not grant dashboard access. All three tokens must use different values. The application does not terminate TLS.
