...
Per-user controls support JSON export, import, and deletion. Format v5 exports include each row's stable record ID, a SHA-256 fingerprint of its normalized contents, and the duration-quality claim used by detail views, and structured artist credits when available. These values let repeated imports distinguish duplicates from conflicting rows and keep reported, estimated, lower-bound, and missing durations consistent after restore. They are not credentials, but they remain part of the sensitive listening-history export and can link copies of the same exported record. Formats v1 through v4 remain importable; v3 keeps its signed confidence value and original comparison rules but cannot recover session/finalization evidence that it never stored, while v1 and v2 receive deterministic identities during import.
