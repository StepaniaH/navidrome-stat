...
5. **`playbackReport`-aware collector upgrade** — When clients send timeline data, capture exact durations/positions/transcode-free session truth (and honor `ignoreScrobble`); degrades gracefully to `minutesAgo` heuristics otherwise. Aligns with the roadmap item "improve collector diagnostics". **M**
...
7. **Upstream watch: `getSongHistory` (PR #5650) adapter seam** — Small abstraction now so a native-history importer lands the day a public endpoint merges; keeps the closed "native import" decision honest and cheap to revisit. **S**
